...
public class GetNuxeoQuery extends AbstractNuxeoProcessor {
	...
	public static final PropertyDescriptor QUERY = new PropertyDescriptor.Builder().name("QUERY").displayName("Query")
			.description("NXQL to execute, nx-query overrides.")
	...
	public static final PropertyDescriptor PAGE_SIZE = new PropertyDescriptor.Builder().name("PAGE_SIZE")
			.displayName("Page Size").description("Page size to retrieve, nx-page-size is used.")
			.expressionLanguageSupported(ExpressionLanguageScope.NONE).defaultValue("10").required(false)
	...
	public static final PropertyDescriptor PAGE_INDEX = new PropertyDescriptor.Builder().name("PAGE_INDEX")
			.displayName("Page Index").description("Page index to start from, nx-page-index overrides.")
			.expressionLanguageSupported(ExpressionLanguageScope.NONE).defaultValue("0").required(false)
			.addValidator(StandardValidators.INTEGER_VALIDATOR).build();

	public static final PropertyDescriptor MAX_RESULTS = new PropertyDescriptor.Builder().name("MAX_RESULTS")
			.displayName("Max Results").description("Max results to return, nx-max-results overrides.")
			.expressionLanguageSupported(ExpressionLanguageScope.NONE).defaultValue("10").required(false)
			.addValidator(StandardValidators.INTEGER_VALIDATOR).build();

	public static final PropertyDescriptor SORT_BY = new PropertyDescriptor.Builder().name("SORT_BY")
			.displayName("Sort By").description("Sort by field, nx-sort-by overrides.")
			.expressionLanguageSupported(ExpressionLanguageScope.NONE).required(false).addValidator(Validator.VALID)
			.build();

	public static final PropertyDescriptor SORT_ORDER = new PropertyDescriptor.Builder().name("SORT_ORDER")
			.displayName("Sort Order").description("Sort order to use (ASC, DESC), nx-sort-order overrides.")
			.expressionLanguageSupported(ExpressionLanguageScope.NONE).required(false).addValidator(Validator.VALID)
			.build();

	public static final PropertyDescriptor QUERY_PARAMS = new PropertyDescriptor.Builder().name("QUERY_PARAMS")
			.displayName("Query Parameters").description("Query parameters to use, nx-query-params overrides.")
			.expressionLanguageSupported(ExpressionLanguageScope.NONE).required(false).addValidator(Validator.VALID)
			.build();
